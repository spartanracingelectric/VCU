...
float dH = 100;
for(int i = 0; i < 500; ++i){
setpointTQ = powerLimit * 9549 / RPM;
float currentError = (float)(setpointTQ - driverTQ);
float proportional = (float)(Kp * currentError);
float integral     = (float)(Ki * (totalError + (currentError * dH)));
float derivative   = (float)(Kd * (currentError - previousError) / dH);

previousError = currentError;
totalError   += currentError * dH;
