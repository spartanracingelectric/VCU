...
CanManager *CanManager_new(ubyte2 can0_busSpeed, ubyte1 can0_read_messageLimit, ubyte1 can0_write_messageLimit,
                           ...
                           SerialManager *serialMan) // ubyte4 defaultMinSendDelay, ubyte4 defaultMaxSendDelay)
{
    ...
    //, the frame size
    //, and other stuff?
    IO_CAN_ConfigFIFO(&me->can0_readHandle, IO_CAN_CHANNEL_0, can0_read_messageLimit, IO_CAN_MSG_READ, IO_CAN_STD_FRAME,
                      0, 0);
    IO_CAN_ConfigFIFO(&me->can0_writeHandle, IO_CAN_CHANNEL_0, can0_write_messageLimit, IO_CAN_MSG_WRITE,
                      IO_CAN_STD_FRAME, 0, 0);
    IO_CAN_ConfigFIFO(&me->can1_readHandle, IO_CAN_CHANNEL_1, can1_read_messageLimit, IO_CAN_MSG_READ, IO_CAN_STD_FRAME,
                      0, 0);
    IO_CAN_ConfigFIFO(&me->can1_writeHandle, IO_CAN_CHANNEL_1, can1_write_messageLimit, IO_CAN_MSG_WRITE,
                      IO_CAN_STD_FRAME, 0, 0);
    ...
    {
        me->canMessageHistory[messageID]->data[i] = 0;
    }
 ...
 ****************************************************************************/
IO_ErrorType CanManager_send(CanManager *me, CanChannel channel, IO_CAN_DATA_FRAME canMessages[],
                             ubyte1 canMessageCount)
{
    ...
    IO_CAN_DATA_FRAME messagesToSend[canMessageCount]; //[channel == CAN0_HIPRI ? me->can0_write_messageLimit :
                                                       //me->can1_write_messageLimit];
    ...
    for (messagePosition = 0; messagePosition < canMessageCount; messagePosition++)
    {
        ...
        //----------------------------------------------------------------------------
        firstTimeMessage = (me->canMessageHistory[outboundMessageID] ==
                            0); // ############################## ALWAYS FALSE? ##############################
        if (firstTimeMessage)
        {
            ...
            for (ubyte1 i = 0; i <= 7; i++)
            {
                me->canMessageHistory[outboundMessageID]->data[i] = 0;
            }
        ...
        //----------------------------------------------------------------------------
        minTimeExceeded =
            ((IO_RTC_GetTimeUS(lastMessage->lastMessage_timeStamp) >= lastMessage->timeBetweenMessages_Min));
        maxTimeExceeded = ((IO_RTC_GetTimeUS(lastMessage->lastMessage_timeStamp) >=
                            50000)); // lastMessage->timeBetweenMessages_Max));
    ...
    if (messagesToSendCount > 0)
    {
        ...
        sendResult = IO_CAN_WriteFIFO((channel == CAN0_HIPRI) ? me->can0_writeHandle : me->can1_writeHandle,
                                      messagesToSend, messagesToSendCount);
        *((channel == CAN0_HIPRI) ? &me->ioErr_can0_write : &me->ioErr_can1_write) = sendResult;
        ...
        if ((channel == CAN0_HIPRI ? me->ioErr_can0_write : me->ioErr_can1_write) == IO_E_OK)
        {
            ...
            for (messagePosition = 0; messagePosition < messagesToSendCount; messagePosition++)
            {
                ...
                /////////////IO_RTC_GetTimeUS(messageToUpdate->lastMessage_timeStamp); //Update the timestamp for when
                ///the message was last sent
 ...
 * read
 ****************************************************************************/
void CanManager_read(CanManager *me, CanChannel channel, MotorController *mcm, InstrumentCluster *ic,
                     BatteryManagementSystem *bms, SafetyChecker *sc)
{
    ...
    *(channel == CAN0_HIPRI ? &me->ioErr_can0_read : &me->ioErr_can1_read) = IO_CAN_ReadFIFO(
        (channel == CAN0_HIPRI ? me->can0_readHandle : me->can1_writeHandle), canMessages,
        (channel == CAN0_HIPRI ? me->can0_read_messageLimit : me->can1_read_messageLimit), &canMessageCount);
    ...
    for (int currMessage = 0; currMessage < canMessageCount; currMessage++)
    {
        ...
        case 0xA0:
        case 0xA1:
        case 0xA2: MCM_parseCanMessage(mcm, &canMessages[currMessage]);
        case 0xA3:
        case 0xA4:
        case 0xA5:
        case 0xA6: MCM_parseCanMessage(mcm, &canMessages[currMessage]);
        case 0xA7: MCM_parseCanMessage(mcm, &canMessages[currMessage]);
        case 0xA8:
        case 0xA9:
        case 0xAA:
        case 0xAB:
        ...
        case 0xAD:
        case 0xAE:
        case 0xAF: MCM_parseCanMessage(mcm, &canMessages[currMessage]); break;
        ...
        case 0x629: BMS_parseCanMessage(bms, &canMessages[currMessage]); break;

        case 0x702: IC_parseCanMessage(ic, mcm, &canMessages[currMessage]); break;
        case 0x703: IC_parseCanMessage(ic, mcm, &canMessages[currMessage]); break;
        case 0x704: IC_parseCanMessage(ic, mcm, &canMessages[currMessage]); break;
...
//
//----------------------------------------------------------------------------
void canOutput_sendDebugMessage(CanManager *me, TorqueEncoder *tps, BrakePressureSensor *bps, MotorController *mcm,
                                InstrumentCluster *ic, BatteryManagementSystem *bms, WheelSpeeds *wss,
                                SafetyChecker *sc, LaunchControl *lc, DRS *drs)
{
    ...
    // tps1Percent = 0xFF * (1 - tempPedalPercent);  //OLD: flipped over pedal percent (this value for display in CAN
    // only)
    ...
    canMessages[canMessageCount - 1].data[byteNum++] =
        brakePercent; // This should be bps0Percent, but for now bps0Percent = brakePercent
    ...
    */
    ...
    canMessages[canMessageCount - 1].data[byteNum++] = 0; // Speed (RPM?) - not needed - mcu should be in torque mode
    canMessages[canMessageCount - 1].data[byteNum++] =
        MCM_commands_getDirection(mcm); // Motor direction (For SR-16: 0 = Forward, 1 = Reverse)
    canMessages[canMessageCount - 1].data[byteNum++] =
        (MCM_commands_getInverter(mcm) == ENABLED)
            ? 1
            : 0; // unused/unused/unused/unused unused/unused/Discharge/Inverter Enable
