...
void canOutput_sendDebugMessage(CanManager* me, TorqueEncoder* tps, BrakePressureSensor* bps, MotorController* mcm, InstrumentCluster* ic, BatteryManagementSystem* bms, WheelSpeeds* wss, SafetyChecker* sc, LaunchControl* lc, DRS *drs)
{
    ...
    */
    ...
    canMessages[canMessageCount - 1].data[byteNum++] = (ubyte1)MCM_getTestTorque(mcm) >> 0;
    canMessages[canMessageCount - 1].data[byteNum++] = (ubyte1)MCM_getTestTorque(mcm) >> 8;
    canMessages[canMessageCount - 1].data[byteNum++] = 0xFF;
    canMessages[canMessageCount - 1].data[byteNum++] = 0;
    canMessages[canMessageCount - 1].data[byteNum++] = 0;
    canMessages[canMessageCount - 1].data[byteNum++] = 0;
    canMessages[canMessageCount - 1].length = byteNum;
    ...
    canMessages[canMessageCount - 1].data[byteNum++] = (ubyte1)MCM_commands_getTorque(mcm) >> 8;
