...
struct _MotorController
{
    ...
    RegenMode regen_mode; // Software reading of regen knob position.  Each mode has different regen behavior (variables
                          // below).
    ubyte2 regen_torqueLimitDNm;       // Tuneable value.  Regen torque (in Nm) at full regen.  Positive value.
    ubyte2 regen_torqueAtZeroPedalDNm; // Tuneable value.  Amount of regen torque (in Nm) to apply when both pedals at
                                       // 0% travel.  Positive value.
    float4 regen_percentBPSForMaxRegen; // Tuneable value.  Amount of brake pedal required for full regen. Value between
                                        // zero and one.
    float4 regen_percentAPPSForCoasting; // Tuneable value.  Amount of accel pedal required to exit regen.  Value
    ...
    //        void(*motorController_setTorque)(MotorController* me, ubyte2 torque); //Will be divided by 10 e.g. pass in
    100 for 10.0 Nm
...
MotorController *MotorController_new(SerialManager *sm, ubyte2 canMessageBaseID, Direction initialDirection,
                                     sbyte2 torqueMaxInDNm, sbyte1 minRegenSpeedKPH, sbyte1 regenRampdownStartSpeed)
{
...
void MCM_calculateCommands(MotorController *me, TorqueEncoder *tps, BrakePressureSensor *bps)
{
    ...
    // appsTorque = me->torqueMaximumDNm * getPercent(appsOutputPercent, me->regen_percentAPPSForCoasting, 1, TRUE) -
    // me->regen_torqueAtZeroPedalDNm * getPercent(appsOutputPercent, me->regen_percentAPPSForCoasting, 0, TRUE);
    // bpsTorque = 0 - (me->regen_torqueLimitDNm - me->regen_torqueAtZeroPedalDNm) * getPercent(bps->percent, 0,
    // me->regen_percentBPSForMaxRegen, TRUE);
...
// See diagram at
// https://onedrive.live.com/redir?resid=F9BB8F0F8FDB5CF8!30410&authkey=!ABSF-uVH-VxQRAs&ithint=file%2chtml
void MCM_inverterControl(MotorController *me, TorqueEncoder *tps, BrakePressureSensor *bps, ReadyToDriveSound *rtds)
{
    ...
    //----------------------------------------------------------------------------
    // New Handshake NOTE: Switches connected to ground.. TRUE = high = off = disconnected = open circuit, FALSE = low =
    // grounded = on = connected = closed circuit
    ...
    case 2: // MCM on, lockout=disabled, inverter=disabled --> stay until RTD button pressed
        ...
        // How to transition to next state ------------------------------------------------
        if (Sensor_RTDButton.sensorValue == FALSE && tps->calibrated == TRUE && bps->calibrated == TRUE &&
    ...
    case 3: // inverted=disabled, rtd=pressed, waiting for inverter to be enabled
        ...
        {
            RTDPercent = 1; // Doesn't matter if button is no longer pressed - RTD light should be on if car is
                            // driveable
            SerialManager_send(me->serialMan, "Inverter has been enabled.  S                                           "
                                              "         tarting RTDS.  Car is ready to drive.\n");
            RTDS_setVolume(rtds, 1, 1500000);
            MCM_setStartupStage(
                me, 4); // MCM_getStartupStage(me) + 1);  //leave this stage since we've already kicked off the RTDS
        }
        break;
    ...
    default: SerialManager_send(me->serialMan, "ERROR: Lost track of MCM startup status.\n"); break;
    }
    ...
    {
    case DISABLED: MCM_commands_setInverter(me, DISABLED); break;
    case ENABLED: MCM_commands_setInverter(me, DISABLED); break;
    case UNKNOWN: break;
    }
...
sbyte2    MCM_commands_getTorque(MotorController *me) { return me->commands_torque; }
Direction MCM_commands_getDirection(MotorController *me) { return me->commands_direction; }
Status    MCM_commands_getInverter(MotorController *me) { return me->commands_inverter; }
Status    MCM_commands_getDischarge(MotorController *me) { return me->commands_discharge; }
sbyte2    MCM_commands_getTorqueLimit(MotorController *me) { return me->commands_torqueLimit; }

void MCM_updateLockoutStatus(MotorController *me, Status newState) { me->lockoutStatus = newState; }
void MCM_updateInverterStatus(MotorController *me, Status newState) { me->inverterStatus = newState; }
...
void MCM_update_LaunchControl_State(MotorController *me, bool newLCState) { me->LCState = newLCState; }

Status MCM_getLockoutStatus(MotorController *me) { return me->lockoutStatus; }

Status MCM_getInverterStatus(MotorController *me) { return me->inverterStatus; }

bool MCM_getHvilOverrideStatus(MotorController *me) { return me->HVILOverride; }

Status MCM_getInverterOverrideStatus(MotorController *me) { return me->InverterOverride; }

void MCM_setRTDSFlag(MotorController *me, bool enableRTDS) { me->startRTDS = enableRTDS; }
...
ubyte2 MCM_commands_getUpdateCount(MotorController *me) { return me->updateCount; }
...
ubyte2 MCM_getTorqueMax(MotorController *me) { return me->torqueMaximumDNm; }

sbyte4 MCM_getPower(MotorController *me) { return ((me->DC_Voltage) * (me->DC_Current)); }

ubyte2 MCM_getCommandedTorque(MotorController *me) { return me->commandedTorque; }
...
sbyte2 MCM_getMotorTemp(MotorController *me) { return me->motor_temp; }
...
ubyte1 MCM_getRegenMode(MotorController *me) { return me->regen_mode; }
sbyte2 MCM_getRegenTorqueLimitDNm(MotorController *me) { return me->regen_torqueLimitDNm; }
sbyte2 MCM_getRegenTorqueAtZeroPedalDNm(MotorController *me) { return me->regen_torqueAtZeroPedalDNm; }
sbyte2 MCM_getRegenBPSForMaxRegenZeroToFF(MotorController *me) { return 0xFF * me->regen_percentBPSForMaxRegen; }
sbyte2 MCM_getRegenAPPSForMaxCoastingZeroToFF(MotorController *me) { return 0xFF * me->regen_percentAPPSForCoasting; }

sbyte1 MCM_getRegenMinSpeed(MotorController *me) { return me->regen_minimumSpeedKPH; }
sbyte1 MCM_getRegenRampdownStartSpeed(MotorController *me) { return me->regen_SpeedRampStart; }

void MCM_setStartupStage(MotorController *me, ubyte1 stage) { me->startupStage = stage; }

ubyte1 MCM_getStartupStage(MotorController *me) { return me->startupStage; }

void MCM_setMaxTorqueDNm(MotorController *me, ubyte2 newTorque) { me->torqueMaximumDNm = newTorque; }
...
ubyte2 MCM_getMaxTorqueDNm(MotorController *me) { return me->torqueMaximumDNm; }
ubyte2 MCM_getRegen_TorqueLimitDNm(MotorController *me) { return me->regen_torqueLimitDNm; }
ubyte2 MCM_getRegen_TorqueAtZeroPedalDNm(MotorController *me) { return me->regen_torqueAtZeroPedalDNm; }
float4 MCM_getRegen_PercentBPSForMaxRegen(MotorController *me) { return me->regen_percentBPSForMaxRegen; }
float4 MCM_getRegen_PercentAPPSForCoasting(MotorController *me) { return me->regen_percentAPPSForCoasting; }