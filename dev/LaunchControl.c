...
LaunchControl *LaunchControl_new(){// this goes outside the while loop
    ...
    me->pidController->total_error = 170; // setting PID total error @ init torque = 170 Nm?
    me->buttonDebug = 0;
    return me;
}
    ...
    // SENSOR_LCBUTTON values are reversed: FALSE = TRUE and TRUE = FALSE, due to the VCU internal Pull-Up for the button and the button's Pull-Down on Vehicle
     ...
     if(me->LCReady == TRUE && Sensor_LCButton.sensorValue == FALSE && tps->travelPercent > .90){
        ...
        if(speedKph > 3){
           ...
           ubyte2 PIDtorque= PID_compute(me->pidController,me->slipRatio);// we erased the saturation checks for now we just want the basic calculation
            me->lcTorque = PIDtorque; // adds the ajusted value from the pid to the torqueval         }
    }
     }
...
ubyte2 getCalculatedTorque(LaunchControl *me){
    return me->lcTorque;
}
