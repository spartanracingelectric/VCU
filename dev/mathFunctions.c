...
* Automatically compensates for reverse-direction values: When min > max, value is assumed to travel in reverse
direction.
...
ubyte2 max(ubyte2 a, ubyte2 b) { return (a > b) ? a : b; }

// A utility function to get maximum of two integers
ubyte2 min(ubyte2 a, ubyte2 b) { return (a < b) ? a : b; }

/**********************************************************************/ /**
                                                                          *
                                                                          * \brief Decides whether a blinking light
                                                                          *should be on or off based on the system clock
                                                                          *and period given
                                                                          *
                                                                          * \param timestamp     A timestamp from IO_RTC
                                                                          *for when the blink was started \param
                                                                          *highPeriod    How long the blink should stay
                                                                          *high, in ms (NOT ns) \param lowPeriod     How
                                                                          *long the blink should stay low -- NOT
                                                                          *IMPLEMENTED; Always the same as highPeriod
                                                                          *
                                                                          * \return boolean:
...
ubyte1 swap_uint8(ubyte1 val) { return (val << 4) | (val >> 4); }

sbyte1 swap_int8(sbyte1 val) { return (val << 4) | (val >> 4); }
ubyte2 swap_uint16(ubyte2 val) { return (val << 8) | (val >> 8); }

//! Byte swap short
sbyte2 swap_int16(sbyte2 val) { return (val << 8) | ((val >> 8) & 0xFF); }
